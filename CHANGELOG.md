...
- Remove `Date` and `DateNullable` types in identity_verification and monitor endpoints. Use `ISO8601Date` and `ISO8601DateNullable`
    instead.
...
- Change `institution_price_as_of` field on the Holdings object to nullable since it's nullable in
    the API.
